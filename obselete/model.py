...
class InteractionNetwork(nn.Module):
    def __init__(
        self, dim_obj, dim_rel, dim_eff, dim_hidden_obj, dim_hidden_rel, dim_x=0
    ):
    ...
    def forward(self, obj, rr, rs, ra, x=None):
        ...
        if x is None:
            a = torch.cat(
                [obj.t(), e.mm(rr.t())]
            )  # shape of a = (obj_dim+eff_dim, n_objects)
        else:
            a = torch.cat(
                [obj.t(), x, e.mm(rr.t())]
            )  # shape of a = (obj_dim+ext_dim+eff_dim, n_objects)
...
class Simulator(torch.nn.Module):
    ...
    def forward(self, P, k):
        ...
        print("No help RMSD:", rmsd(P.pos, P.pos + (P.vels * 20))[0])
