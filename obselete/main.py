...
parser_makeinput = subparsers.add_parser(
    "makeinput",
    description="Form an input protein data file.",
    help="form an input protein data file",
)
parser_makeinput.add_argument(
    "-i", "--input", required=True, help="PDB/mmCIF file, format guessed from extension"
)
parser_makeinput.add_argument(
    "-s", "--ss2", help="PSIPRED ss2 file, default fully coiled (not recommended)"
)
...
parser_simulate.add_argument(
    "-i", "--input", required=True, help="input protein data file"
)
parser_simulate.add_argument(
    "-o",
    "--output",
...
parser_simulate.add_argument(
    ...
    "--startconf",
    required=True,
    choices=["native", "extended", "predss", "random", "helix"],
    help="starting conformation",
)
parser_simulate.add_argument(
    "-n",
    "--nsteps",
...
parser_simulate.add_argument(
    "-p", "--parameters", help="parameter set to use, default is the trained model"
)
parser_simulate.add_argument(
    "-d",
    "--device",
...
parser_thread = subparsers.add_parser(
    "thread",
    description=(
        "Calculate the energy in the learned potential of a set of sequences threaded "
        "onto a structure. Returns one energy per line."
    ),
    help=(
        "calculate the energy in the learned potential of a set of sequences threaded onto a "
        "structure"
    ),
)
parser_thread.add_argument(
    "-i", "--input", required=True, help="input protein data file"
)
parser_thread.add_argument(
    "-s", "--sequences", required=True, help="file of sequences to thread, one per line"
)
parser_thread.add_argument(
    "-m",
    "--minsteps",
...
for arg in (
    ...
    if arg in args.__dict__ and args.__dict__[arg] < 0:
        raise argparse.ArgumentTypeError(
            f"{arg} is {args.__dict__[arg]} but must be non-negative"
        )
...
if args.mode == "makeinput":
    ...
    native_coords, inters_flat, inters_ang, inters_dih, masses, seq = read_input_file(
        args.input, device=device
    )
    with torch.no_grad():
        simulator = Simulator(
            params["distances"], params["angles"], params["dihedrals"]
        )
        if args.startconf == "native":
            coords = native_coords
        else:
            coords = starting_coords(
                seq, conformation=args.startconf, input_file=args.input, device=device
            )
        coords = simulator(
            coords.unsqueeze(0),
            ...
            start_temperature=args.starttemperature,
            thermostat_const=args.coupling,
            temperature=args.temperature,
            sim_filepath=args.output,
            report_n=args.report,
