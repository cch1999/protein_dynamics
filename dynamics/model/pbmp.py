...
class PBMP(nn.Module):
    def __init__(
        ...
        self.distance_forces = DistanceForces(
            input_dim + 2, hidden_dim, num_hidden_layers, 1
        )
        self.angle_forces = AngleForces(
            input_dim * 3 + 1, hidden_dim, num_hidden_layers, 1
        )
        self.dihedral_forces = DihedralForces(
            input_dim * 4 + 1, hidden_dim, num_hidden_layers, 1
        )
