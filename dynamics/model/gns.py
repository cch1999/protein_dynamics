...
class MPNN(nn.Module):
	...
	def __init__(self, edge_mlp, node_mlp):
		...
		self.edge_mlp = MLP_with_layer_norm(edge_mlp.input_size*3,
	...
	def forward(self, P):
		# Messages
		m = self.edge_mlp(torch.cat([P.x[P.receivers], P.x[P.senders], P.edge_attr], -1))

		# Aggregate
		x = (m.view(P.n_atoms, 128, k)).sum(-1)
...
class GNS(nn.Module):
	...
	def _update(self, P):
		...
		P.pos = P.pos + P.vels * self.timestep + 0.5 * P.accs * self.timestep * self.timestep
