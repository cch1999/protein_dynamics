...
class Simulator(torch.nn.Module):
    ...
    def forward(self,
                ...
                # Set to 0.0 to run without a thermostat (NVE ensemble)
                thermostat_const=0.0,
        ...
        assert integrator in ("vel", "no_vel", "min", "langevin",
                              "langevin_simple"), f"Invalid integrator {integrator}"
        ...
        pair_centres_flat = dist_bin_centres_tensor.index_select(
            0, inters_flat[0]).unsqueeze(0).expand(batch_size, -1, -1)
        pair_pots_flat = self.ff_distances.index_select(
            0, inters_flat[0]).unsqueeze(0).expand(batch_size, -1, -1)
        angle_bin_centres_tensor = torch.tensor(
            angle_bin_centres, device=device)
        angle_centres_flat = angle_bin_centres_tensor.unsqueeze(
            0).unsqueeze(0).expand(batch_size, n_res, -1)
        angle_pots_flat = self.ff_angles.index_select(
            1, inters_ang[0]).unsqueeze(0).expand(batch_size, -1, -1, -1)
        dih_bin_centres_tensor = torch.tensor(dih_bin_centres, device=device)
        dih_centres_flat = dih_bin_centres_tensor.unsqueeze(
            0).unsqueeze(0).expand(batch_size, n_res - 1, -1)
        dih_pots_flat = self.ff_dihedrals.index_select(
            1, inters_dih[0]).unsqueeze(0).expand(batch_size, -1, -1, -1)
        native_coords_ca = native_coords.view(
            batch_size, n_res, 3 * len(atoms))[0, :, 3:6]
        model_n = 0
        ...
        if integrator == "vel" or integrator == "langevin" or integrator == "langevin_simple":
            ...
            coords_last = coords.clone() + torch.randn(coords.shape, device=device) * \
                start_temperature * timestep
        ...
        for i in range(n_steps):
            if integrator == "vel":
                ...
                beta = np.sqrt(twokbT * alpha * timestep) * \
                    torch.randn(vels.shape, device=device)
                b = 1.0 / (1.0 + (alpha * timestep) /
                           (2 * masses.unsqueeze(2)))
                coords_last = coords
                coords = coords + b * timestep * vels + 0.5 * b * \
                    (timestep ** 2) * accs_last + 0.5 * b * \
                    timestep * beta / masses.unsqueeze(2)
            ...
            dists_from_centres = pair_centres_flat - \
            ...
            pair_forces_flat = 0.5 * \
                (pair_pots_flat.gather(2, dist_bin_inds) -
                 pair_pots_flat.gather(2, dist_bin_inds + 2))
            # Specify minimum to prevent division by zero errors
            norm_diffs = diffs / dists.clamp(min=0.01).unsqueeze(3)
            pair_accs = (pair_forces_flat.view(
                batch_size, n_atoms, n_atoms)).unsqueeze(3) * norm_diffs
            accs = pair_accs.sum(dim=1) / masses.unsqueeze(2)
            if printing or returning_energy:
                dist_energy += 0.5 * \
                    pair_pots_flat.gather(2, dist_bin_inds + 1).sum()

            atom_coords = coords.view(batch_size, n_res, 3 * len(atoms))
            atom_accs = torch.zeros(
                batch_size, n_res, 3 * len(atoms), device=device)
            # Angle forces
            ...
            for ai, (atom_1, atom_2, atom_3, across_res) in enumerate(angles):
                ai_1, ai_2, ai_3 = atoms.index(atom_1), atoms.index(
                    atom_2), atoms.index(atom_3)
                if across_res == 0:
                    ba = atom_coords[:, :, (ai_1 * 3):(ai_1 * 3 + 3)] - \
                        atom_coords[:, :, (ai_2 * 3):(ai_2 * 3 + 3)]
                    bc = atom_coords[:, :, (ai_3 * 3):(ai_3 * 3 + 3)] - \
                        atom_coords[:, :, (ai_2 * 3):(ai_2 * 3 + 3)]
                    # Use residue potential according to central atom
                    angle_pots_to_use = angle_pots_flat[:, ai, :]
                elif across_res == 1:
                    ba = atom_coords[:, :-1, (ai_1 * 3):(ai_1 * 3 + 3)] - \
                        atom_coords[:, :-1, (ai_2 * 3):(ai_2 * 3 + 3)]
                    bc = atom_coords[:, 1:, (ai_3 * 3):(ai_3 * 3 + 3)] - \
                        atom_coords[:, :-1, (ai_2 * 3):(ai_2 * 3 + 3)]
                ...
                angles_from_centres = angle_centres_flat[:,
                                                         :n_angles] - angs.unsqueeze(2)
                angle_bin_inds = angles_from_centres.abs().argmin(dim=2).unsqueeze(2)
                angle_forces = 0.5 * (angle_pots_to_use.gather(2, angle_bin_inds) -
                                      angle_pots_to_use.gather(2, angle_bin_inds + 2))
                cross_ba_bc = torch.cross(ba, bc, dim=2)
                fa = angle_forces * \
                    normalize(torch.cross(ba, cross_ba_bc, dim=2),
                              dim=2) / ba_norms.unsqueeze(2)
                fc = angle_forces * \
                    normalize(torch.cross(-bc, cross_ba_bc, dim=2),
                              dim=2) / bc_norms.unsqueeze(2)
                fb = -fa - fc
                if across_res == 0:
                    ...
                    atom_accs[:, 1:, (ai_3 * 3):(ai_3 * 3 + 3)] += fc
                if printing or returning_energy:
                    angle_energy += angle_pots_to_use.gather(
                        2, angle_bin_inds + 1).sum()
            ...
            for di, (atom_1, atom_2, atom_3, atom_4, across_res) in enumerate(dihedrals):
                ai_1, ai_2, ai_3, ai_4 = atoms.index(atom_1), atoms.index(
                    atom_2), atoms.index(atom_3), atoms.index(atom_4)
                if across_res == 1:
                    ab = atom_coords[:, :-1, (ai_2 * 3):(ai_2 * 3 + 3)] - \
                        atom_coords[:, :-1, (ai_1 * 3):(ai_1 * 3 + 3)]
                    bc = atom_coords[:, :-1, (ai_3 * 3):(ai_3 * 3 + 3)] - \
                        atom_coords[:, :-1, (ai_2 * 3):(ai_2 * 3 + 3)]
                    cd = atom_coords[:, 1:, (ai_4 * 3):(ai_4 * 3 + 3)] - \
                ...
                dihs = torch.atan2(
                    torch.sum(torch.cross(cross_ab_bc, cross_bc_cd,
                                          dim=2) * bc / bc_norms, dim=2),
                ...
                dih_forces = 0.5 * (dih_pots_to_use.gather(2, dih_bin_inds) -
                                    dih_pots_to_use.gather(2, dih_bin_inds + 2))
                fa = dih_forces * \
                    normalize(-cross_ab_bc, dim=2) / \
                    ab.norm(dim=2).unsqueeze(2)
                fd = dih_forces * \
                    normalize(cross_bc_cd, dim=2) / cd.norm(dim=2).unsqueeze(2)
                ...
                fb = ((ab * -bc) / (bc_norms ** 2) - 1) * \
                    fa - ((cd * -bc) / (bc_norms ** 2)) * fd
                fc = -fa - fb - fd
                if across_res == 1:
                    atom_accs[:, :-1, (ai_1 * 3):(ai_1 * 3 + 3)] += fa
                    atom_accs[:, :-1, (ai_2 * 3):(ai_2 * 3 + 3)] += fb
                    ...
                    atom_accs[:, 1:, (ai_4 * 3):(ai_4 * 3 + 3)] += fd
                if printing or returning_energy:
                    dih_energy += dih_pots_to_use.gather(
                        2, dih_bin_inds + 1).sum()

            accs += atom_accs.view(batch_size, n_atoms,
                                   3) / masses.unsqueeze(2)
            ...
            if integrator == "vel":
                ...
                # From Gronbech-Jensen 2013
                vels = vels + 0.5 * timestep * (accs_last + accs) - alpha * (
                    coords - coords_last) / masses.unsqueeze(2) + beta / masses.unsqueeze(2)
                accs_last = accs
            elif integrator == "langevin_simple":
                gamma, twokbT = thermostat_const, temperature
                accs = accs + (-gamma * vels + np.sqrt(gamma * twokbT) *
                               torch.randn(vels.shape, device=device)) / masses.unsqueeze(2)
                vels = vels + 0.5 * (accs_last + accs) * timestep
                accs_last = accs
            elif integrator == "min":
                ...
                for ai in range(n_atoms):
                    if random() < thermostat_prob:
                        if integrator == "vel":
                            ...
                            new_vel = torch.randn(
                                3, device=device) * temperature
                            vels[0, ai] = new_vel
                        elif integrator == "no_vel":
                            new_diff = torch.randn(
                                3, device=device) * temperature * timestep
            ...
            if printing:
                ...
                out_line = "    Step {:8} / {} - acc {:6.3f} {}- energy {:6.2f} ( {:6.2f} {:6.2f} {:6.2f} ) - Cα RMSD {:6.2f}".format(
                    ...
                    "- vel {:6.3f} ".format(torch.mean(vels.norm(dim=2)).item()) if integrator in (
                        "vel", "langevin", "langevin_simple") else "",
